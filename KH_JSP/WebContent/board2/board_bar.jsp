...
<c:if test="${member_info eq null}">
		<a href="login?id=${id}&pages=${pages}">로그인</a>
		&nbsp;
		<a href="join?id=join&pages=${pages}">회원가입</a>
</c:if>
<c:if test="${!(member_info eq null)}">
		<span style="font-weight:bold;">${member_info.name}</span>님 환영합니다.
		<a href="join_edit?id=join&pages=${pages}">[정보수정]</a>
		<a href="logout?id=${id}&pages=${pages}">[로그아웃]</a>
</c:if>
	</p>
</div>