...
<div class="write_hide_scroll">
    ...
    <input type="hidden" name="pages_rc" value="${pages_rc}" />
    <input type="hidden" name="no" value="${rcdata.no}" />
    <input type="hidden" name="pages" value="${pages}" />
    <input type="hidden" name="member_no" value="${member_no}" />
    <input type="hidden" name="searchValue" value="${searchValue_utf}" />
    <input type="hidden" name="search" value="${search}" />
...
<div class="container">
  <div class="row">
    ...
    <div class="review_write">
    	...
    	<c:if test="${memberInfo eq null}"><a href="login.o" style="background:#219bf0;border:1px solid #219bf0;">지원하기</a></c:if>
    	<c:if test="${memberInfo ne null}">
    		<c:if test="${memberInfo.orders eq 1}"><a href="#100" onclick="show2('recruit_add');document.body.style.overflow = 'hidden';" style="background:#219bf0;border:1px solid #219bf0;">지원하기</a></c:if>
    	</c:if>
    	</c:if>
    ...
    <div class="recruit_subject">
    	<div class="recruit_dates">
    		<span>D-${count5}</span> ${rcdata.dates} ~ ${rcdata.enddates}
    		&nbsp;&nbsp;
    		<div>${rcdata.dates} 등록</div>
    		<c:if test="${memberInfo.no eq member_no}">
	    		<a href="#100" onclick="show2('recruit_edit');document.body.style.overflow = 'hidden';" class="recruit_a">수정</a>
	    		<a href="#100" onclick="dialog('정말로 삭제하시겠습니까?','recruit_del.o?member_no=${member_no}&no=${rcdata.no}&searchValue=${searchValue_utf}&pages=${pages}&search=${search}&searchType=${searchType}&searchSort=${searchSort}');" class="recruit_a">삭제</a>
	    	</c:if>
    	</div>
